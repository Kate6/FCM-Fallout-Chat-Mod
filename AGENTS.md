...
in the loader menu. The overlay owns bridge authentication. Preserve the existing visible HUD
architecture, authentication and native protocol; adapt the bridge to it. Native HUD and local-export bridge
observations must use one canonical room coordinator, history and publication service across
ZFE/xScal. Roster names are evidence, never account authentication. Preserve per-session authority,
freshness, generation checks and canonical-ID deduplication. Never coinstall with the visible
widget or legacy FCMBridge. See the bridge guide for deployment/native acceptance status.
