...
gates, packages it with the PROD bridge, uploads and size-verifies the EXE and ZIP,
then records `portableDownloadUrl`. The Install page presents the portable ZIP next
to the installer; Discord and GitHub release messages include it when available.
Packaging alone still does not publish, announce, or install anything.
...
- `Fallout Chat Mod Portable X.Y.Z.zip` — portable EXE, README, checksums, and the
  optional manual PROD bridge package. It never installs or modifies game files.
  Packaging fails closed unless the bridge ZIP is the native-accepted 0.2.4 artifact
  with SHA-256 `d2e229b601977efead12108d3db1a09b622f6580fe4f4b2ee50fd0a3d3f3ae47`.
