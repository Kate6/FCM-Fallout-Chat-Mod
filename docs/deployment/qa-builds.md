...
- **"Could not create your session" after the role gate passes:** a `User.discordId`
  (`@map("discord_id_link")`) or `User.username` `@unique` collision in the QA OAuth
  callback. The callback detaches the Discord ID from **any** other install before linking
  (`upsertUser`, scoped only by `NOT: { installToken }`) and creates each fresh QA row with
  a unique non-displayable `pending-qa-*` username, so returning testers can authorize a
  new install without colliding with the detached row. Check the dev backend log for
  `[qa-oauth] failed to issue QA session` (Prisma `P2002`) when diagnosing older deployments.
