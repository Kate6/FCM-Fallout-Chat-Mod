...
| `supporterService.ts` | Entitlement read/write, Redis-cached tier lookup, and the entitlement-vs-privileges split. The configured `ADMIN_ROLE_ID`, plus authenticated `owner`/`admin` identities (including hosted-DEV System Admin), resolve to the full Overseer cosmetics tier without changing `EffectiveRole`. |
| `supporterSyncService.ts` | Keeps entitlements in lockstep with Discord tier/admin cosmetics roles: `guildMemberUpdate` / `guildMemberRemove`, an immediate startup + 15-minute bulk reconcile, and a Redis-coordinated once-per-minute-per-account live refresh used by login, link-status, overlay/dashboard reads, Discord `/cosmetics` interactions, and authenticated HUD sends. Replicas that do not own the refresh slot still invalidate the shared tier/cosmetics read caches before decorating a HUD send, preventing transient stale `none` projections. |
