...
**FAIL-CLOSED ORDER:** build installer + portable -> **smoke-test both executables (gate)** -> **VirusTotal both executables (gate, must pass)** -> build ZIPs -> upload artifacts -> verify served sizes -> Nexus -> `POST /admin/releases`. Nexus runs before release registration so a missing or failed Nexus upload cannot advertise a release in the client feed. These external operations are still not one transaction: if registration fails after Nexus succeeds, retry registration for the same version rather than uploading a second Nexus file.

**Preferred: use the orchestrator** -- `Packaging/release.ps1` enforces the full sequence automatically and cannot skip gates:

```powershell
.\Packaging\release.ps1 -Version X.Y.Z -BridgeZip "<validated-0.2.4-prod.zip>" [-ReleaseNotes "..."] [-SkipBuild] [-DryRun] [-SkipWindowsNexus]
...
```powershell
# After Step 1 (build), gate both Windows executables before any publishing:
...
.\Packaging\smoke-test.ps1 -Version X.Y.Z -Artifact Portable
.\Packaging\vt-gate.ps1 -Version X.Y.Z -ExePath "...\Fallout Chat Mod Portable X.Y.Z.exe" -SkipPermalinkUpdate
```
...
This creates a folder and ZIP containing the portable EXE, README.txt,
SHA256SUMS.txt, and an explicitly optional `Optional FCM Bridge/` directory. It
never copies profiles, credentials, or the visible HUD mod. The bridge remains a
manual opt-in and is never installed by the overlay.
...
The release orchestrator runs the exact portable EXE through smoke and VirusTotal
gates, uploads and size-verifies the EXE and ZIP, then records
`portableDownloadUrl`. Every overlay ZIP includes the validated bridge under a
clearly optional folder with separate ZFE/xScal sections. The Install page presents
the portable ZIP next to the installer; packaging alone does not publish or install.
...
```powershell
.\Packaging\package-downloads.ps1 -Version X.Y.Z -BridgeZip "<validated-0.2.4-prod.zip>"
```

Produces (in `cross-platform-overlay/dist-electron/`):
- `Fallout Chat Mod Setup X.Y.Z (Windows).zip` — installer guide plus optional bridge folder.
- `Fallout Chat Mod Portable X.Y.Z.zip` — portable guide plus optional bridge folder.
  Packaging fails closed unless the bundled bridge is the native-accepted artifact with SHA-256
  `d2e229b601977efead12108d3db1a09b622f6580fe4f4b2ee50fd0a3d3f3ae47`.
- `Fallout Chat Mod-X.Y.Z.AppImage (Linux).zip` — bundles the AppImage, `.deb`,
  Linux guide, `.kwinrule`, and clearly optional bridge folder.
- `FCM HUD Mod-<widget-version> (PROD).zip` — generated by
  `game-mods/FCMBridge/hudmodloader-chat/package.py` from the current `FCMChatWidget.hx`
  version. It contains the BA2, shared UI INI, provider configuration examples, and the append-only
...
both overlay ZIPs, and the `FCM HUD Mod` ZIP, then verifies served sizes (step 5) for each.
...
curl -X POST https://falloutchatmod.com/admin/releases \
  ...
  -d '{"version":"X.Y.Z","downloadUrl":"https://falloutchatmod.com/downloads/electron/Fallout%20Chat%20Mod%20Setup%20X.Y.Z%20(Windows).zip","portableDownloadUrl":"https://falloutchatmod.com/downloads/electron/Fallout%20Chat%20Mod%20Portable%20X.Y.Z.zip","hudModVersion":"<widget-version>","hudModUrl":"https://falloutchatmod.com/downloads/electron/FCM%20HUD%20Mod-<widget-version>%20(PROD).zip","releaseNotes":"...","releaseTarget":"both"}'
```

`downloadUrl` is the Windows installer ZIP URL. `portableDownloadUrl` is optional for
older records and drives the adjacent portable button when present.
...
Build one package with `package.py --target dev --output "FCM HUD Mod-<version> (DEV).zip"`.
The default unified archive contains one BA2 and both ZFE/xScal configuration examples.
No active ZFE fragment is installed on extraction; users apply only their extender's example.
Set `hudModUrl` to this single ZIP. Older sibling-pair URLs remain supported for compatibility,
but new releases ship one download. xScal's included helper enables chat and backs up the INI.
