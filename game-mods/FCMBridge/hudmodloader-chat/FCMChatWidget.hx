...
class FCMChatWidget extends MovieClip {
    ...
    // Reused display objects: at most two staging layers (double-buffer) plus one
    // shared slice timer. Rows themselves are reused via prefix reparenting.
    ...
    function stopSliceTimer():Void {
        if (_sliceTimer != null) {
            ...
            // Kept for reuse across renders; listeners are one-shot closures.
        }
    }
    ...
    function visibleRecordKeys(visible:Array<ChatRecord>):Array<String> {
        ...
        for (rec in visible) {
            keys.push(FcmFeedPlan.recordKey(rec.channel, rec.messageId, rec.localSendId, rec.pending,
                rec.body, rec.linkUrl));
        }
        return keys;
    }
    ...
    function commitFeedSnapshot(renderToken:Int, layer:Sprite, rows:Array<FeedRowView>, contentHeight:Float,
            ?recordKeys:Array<String>, viewportWidth:Float = -1):Void {
    ...
    function renderRecords():Void {
        ...
        var reusedRows:Array<FeedRowView> = null;
        ...
        if (reuseCount > 0 && _feedContentLayer != null) {
            try {
                for (i in 0...reuseCount) {
                    ...
                    try { _feedContentLayer.removeChild(kept.view); } catch (_:Dynamic) {}
                    kept.contentY = contentY;
                    kept.view.y = contentY;
                    stagingRows.push(kept);
                    stagingLayer.addChild(kept.view);
                    contentY += kept.height + FEED_ROW_GAP;
                }
                reusedRows = stagingRows.copy();
            } catch (_:Dynamic) {
                // Reparenting must never break the feed; fall back to a full build.
                try {
                    while (stagingLayer.numChildren > 0) stagingLayer.removeChildAt(0);
                } catch (_:Dynamic) {}
                stagingRows = [];
                contentY = 0;
                reuseCount = 0;
        ...
        if (remaining <= _renderSliceSize) {
            ...
            _renderPending = true;
            commitFeedSnapshot(renderToken, stagingLayer, stagingRows, contentY, newKeys, viewportWidth);
        } else {
            ...
            doSlice = function():Void {
                ...
                if (renderedCount < pendingRecords.length) {
                    ...
                    sliceRef.addEventListener(TimerEvent.TIMER_COMPLETE, function(_:Dynamic) {
