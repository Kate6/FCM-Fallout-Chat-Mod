...
5. **Provider OAuth link flows** — `discord:link` opens the authorization URL with `shell.openExternal` (never an Electron `BrowserWindow`) → bounded status polling observes linked=true → re-register → authenticated relay:status. QA Discord login uses the same external-browser handoff and its role-gated status poll. Steam retains its existing provider-specific flow.
